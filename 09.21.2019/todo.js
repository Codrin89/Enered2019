...
function initPage() {
	...
	for(let i = 0; i < getData.length; i++) {
		container.innerHTML += '<div class="row"><div class="column">'+getData[i].name+'</div><div class="column">'+getData[i].hour+'</div><div class="column">'+getData[i].day+'</div><div class="column large">'+getData[i].todo+'</div><div class="column"><button class="btn-update" data-id="'+i+'"><i class="fa fa-pencil"></i> Update</button><button class="btn-delete" data-id="'+i+'"><i class="fa fa-trash-o"></i> Delete</button></div></div>';
	}
}
...
document.getElementById('btn-sortDate').addEventListener('click', function(event) {
	const data = JSON.parse(window.localStorage.getItem('todo-data'));
	data.sort(function(a, b){
		if (a.date < b.date) {
			return -1;
