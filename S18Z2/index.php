...
$app->post('/login', function () use ($app, $dbh) {
    ...
    $query = "SELECT * FROM `users` WHERE `password`='".$result['password']."' AND (`name`='".$result['username']."' OR `email`='".$result['username']."')";
    $resultInfo = mysqli_query($dbh, $query); 
    $info = mysqli_fetch_assoc($resultInfo);
    if($info != NULL) {
        $token = generateRandomString();
        $queryUpdate = "UPDATE `users` SET `token`='".$token."' WHERE (`name`='".$result['username']."' OR `email`='".$result['username']."')";
...
$app->post('/register', function () use ($app, $dbh) {
    $json = $app->request->getBody();
    $result = json_decode($json, true);
    $query = "SELECT * FROM `users` WHERE `name`='".$result['username']."' OR  `email`='".$result['email']."'";
    $resultQuery = mysqli_query($dbh, $query);
    $infoQuery = mysqli_fetch_assoc($resultQuery);
    if($infoQuery == NULL) {
        $queryUpdate = "INSERT INTO `users` (`name`, `email`, `password`) VALUES ('".$result['username']."', '".$result['email']."', '".$result['password']."')";
