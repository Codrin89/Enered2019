...
function initPage() {
	...
	for(let i=0; i < getData.length; i++) {
		container.innerHTML += '<div class="row"><div class="column">' +getData[i].name+ '</div><div class="column">' +getData[i].hour+'</div><div class="column">' +getData[i].day+'</div><div class="column">'+getData[i].todo+ '</div><button class="btn-update" data-id="'+i+'">Update</button><button class="btn-delete" data-id="'+i+'">Delete</button></div>'; 
	}
...
const data = JSON.parse(window.localStorage.getItem(' item-update'));
